...
namespace Infra.Repositorio
{
    public class RepositorioUsuarioSistemaFinanceiro: RepositoryGeneric<UsuarioSistemaFinanceiro>, InterfaceUsuarioSistemaFinanceiro
    {
        ...
        public async Task<UsuarioSistemaFinanceiro> ObterUsuarioPorEmail(string emailUsuaruio)
        {
            using (var banco = new ContextBase(_OptionsBuilder))
            {
                return await
                    banco.UsuarioSistemaFinanceiro.AsNoTracking().FirstOrDefaultAsync(x => x.EmailUsuario.Equals(emailUsuaruio));
            }
        }
